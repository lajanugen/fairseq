...
if [ $EVAL == "1" ]; then
	ARGS="$ARGS --eval-num-iter-start $EVAL_START --eval-num-iter-end $EVAL_END --eval-num-iter-step $EVAL_STEP "
fi
...
ARGS="$ARGS \
	...
	--load_from_pickle \
	--load_train_tasks_file /checkpoint/annl/transfer_learn_composite_grid/task_lists/train.sample68.pkl \
	--load_test_tasks_file  /checkpoint/annl/transfer_learn_composite_grid/task_lists/$TEST_FILE \
