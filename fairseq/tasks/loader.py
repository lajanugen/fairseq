...
def split_by_labels(data):
    ...
    for label in unique_labels:
        ...
        mask = labels == label
        for key in ['text', 'text_len', 'label', 'raw']:
            data_by_category[label][key] = data[key][mask]
        # for key in ['vocab_size', 'is_train']:
        for key in ['vocab_size']:
            data_by_category[label][key] = data[key]
...
def load_dataset(args):
    ...
    tprint('Loading data')
    all_data = _load_json(args.data_path + "/" + args.dataset + ".json")
    ...
    vocab = Vocab(collections.Counter(_read_words(all_data)), vectors=vectors,
                  specials=['<pad>', '<unk>', '[CLS]'], min_freq=5)
    ...
    train_data = split_by_labels(train_data)
    val_data = split_by_labels(val_data)
    test_data = split_by_labels(test_data)
