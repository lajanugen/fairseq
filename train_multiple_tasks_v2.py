...
def cross_validate(stats, no_training):
    ...
    for i in range(num_tasks):
        ...
        task_train_accuracies = [float(train_accs[i]['post_accuracy_train']) for i in range(len(train_accs))]
        task_val_accuracies = [float(val_accs[i]['post_accuracy_train']) for i in range(len(val_accs))]
        task_val_accuracies_all.append(task_val_accuracies)

        best_train_accuracies.append(max(task_train_accuracies))
        val_accuracies.append(max(task_val_accuracies))

    task_val_accuracies_mean = np.mean(task_val_accuracies_all, axis=0)
    best_num_iters = np.argmax(task_val_accuracies_mean)
...
def cli_main(args, examples, state):
    ...
    if args.distributed_init_method is not None:
        ...
        # single GPU training
        train_stats, valid_stats = main(args, examples, state)
